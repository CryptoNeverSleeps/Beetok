...
VERSION=
commit=false
url=https://github.com/altbetproject/beetok
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the beetok, gitian-builder, gitian.sigs, and beetok-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/altbetproject/beetok
...
then
	# Make output folder
	mkdir -p "./beetok-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit beetok=${COMMIT} --url beetok=${url} ../beetok/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../beetok/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/beetok-*.tar.gz build/out/src/beetok-*.tar.gz ../beetok-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit beetok=${COMMIT} --url beetok=${url} ../beetok/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/beetok-*.zip build/out/beetok-*.exe ../beetok-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit beetok=${COMMIT} --url beetok=${url} ../beetok/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/beetok-*.tar.gz build/out/beetok-*.dmg ../beetok-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit beetok=${COMMIT} --url beetok=${url} ../beetok/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../beetok/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/beetok-*.tar.gz build/out/src/beetok-*.tar.gz ../beetok-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../beetok/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/beetok-*win64-setup.exe ../beetok-binaries/${VERSION}
	    mv build/out/beetok-*win32-setup.exe ../beetok-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../beetok/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/beetok-osx-signed.dmg ../beetok-binaries/${VERSION}/beetok-${VERSION}-osx.dmg
	fi
	popd || exit
