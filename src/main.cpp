...
#if defined(NDEBUG)
#error "Beetok cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zBTOK is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zBTOK spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zBTOK without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zBTOK spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zBTOK spend with serial %s from tx %s is not in valid range\n", __func__,
...

            {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                        return state.Invalid(error("AcceptToMemoryPool : zBTOK spend tx %s already in block %d",
                    ...
                    for (const CTxIn& txIn : tx.vin) {
                        ...
                        if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                            return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                     tx.GetHash().GetHex()),
                                REJECT_INVALID, "bad-txns-invalid-zbtok");
                    }
                } else {
                    LOCK(pool.cs);
                    ...
                    // Check that zBTOK mints are not already known
                    if (tx.IsZerocoinMint()) {
                        for (auto& out : tx.vout) {
                            if (!out.IsZerocoinMint())
                                continue;
...
int64_t GetBlockValue(int nHeight)
    ...
    int64_t nSubsidy = 0;
            if (nHeight == 0) {nSubsidy = 820000 * COIN;} 
			else if (nHeight <= 200){nSubsidy = 0 * COIN;} 
			else if (nHeight <= 40000) {nSubsidy = 1 * COIN;} 
			else if (nHeight <= 50000) { nSubsidy = 10 * COIN;} 
			else if (nHeight <= 60000) {nSubsidy = 4.25 * COIN;} 
			else if (nHeight <= 70000) {nSubsidy = 4.5 * COIN;} 
			else if (nHeight <= 80000) {nSubsidy = 4.75 * COIN;} 
			else if (nHeight <= 90000) {nSubsidy = 5 * COIN;} 
			else if (nHeight <= 100000) {nSubsidy = 5.25 * COIN;} 
			else if (nHeight <= 110000) {nSubsidy = 5.5 * COIN;} 
			else if (nHeight <= 120000) {nSubsidy = 5.25 * COIN;} 
			else if (nHeight <= 130000) {nSubsidy = 5.5 * COIN;} 
			else if (nHeight <= 140000) {nSubsidy = 5.75 * COIN;} 
			else if (nHeight <= 150000) {nSubsidy = 6 * COIN;} 
			else if (nHeight <= 160000) {nSubsidy = 6.25 * COIN;} 
			else if (nHeight <= 170000) {nSubsidy = 6.5 * COIN;} 
			else if (nHeight <= 180000) {nSubsidy = 6.75 * COIN;} 
			else if (nHeight <= 190000) {nSubsidy = 7 * COIN;} 
			else if (nHeight <= 200000) {nSubsidy = 7.25 * COIN;} 
			else if (nHeight <= 210000) {nSubsidy = 7.5 * COIN;} 
			else if (nHeight <= 220000) {nSubsidy = 7.75 * COIN;} 
			else if (nHeight <= 230000) {nSubsidy = 8 * COIN;} 
			else if (nHeight <= 240000) {nSubsidy = 8.25 * COIN;} 
			else if (nHeight <= 250000) {nSubsidy = 8.5 * COIN;} 
			else if (nHeight <= 260000) {nSubsidy = 8.75 * COIN;} 
			else if (nHeight <= 270000) {nSubsidy = 9 * COIN;} 
			else if (nHeight <= 280000) {nSubsidy = 9.25 * COIN;} 
			else if (nHeight <= 290000) {nSubsidy = 9.5 * COIN;} 
			else if (nHeight <= 300000) {nSubsidy = 9.75 * COIN;} 
			else if (nHeight <= 310000) {nSubsidy = 10 * COIN;} 
			else if (nHeight <= 320000) {nSubsidy = 9.75 * COIN;} 
			else if (nHeight <= 330000) {nSubsidy = 9.5 * COIN;} 
			else if (nHeight <= 340000) {nSubsidy = 9.25 * COIN;} 
			else if (nHeight <= 350000) {nSubsidy = 9 * COIN;} 
			else if (nHeight <= 360000) {nSubsidy = 8.75 * COIN;} 
			else if (nHeight <= 370000) {nSubsidy = 8.5 * COIN;} 
			else if (nHeight <= 380000) {nSubsidy = 8.25 * COIN;} 
			else if (nHeight <= 390000) {nSubsidy = 8 * COIN;} 
			else if (nHeight <= 400000) {nSubsidy = 7.75 * COIN;}
			else if (nHeight <= 410000) {nSubsidy = 7.5 * COIN;} 
			else if (nHeight <= 420000) {nSubsidy = 7.25 * COIN;} 
			else if (nHeight <= 430000) {nSubsidy = 7 * COIN;} 
			else if (nHeight <= 440000) {nSubsidy = 6.75 * COIN;} 
			else if (nHeight <= 450000) {nSubsidy = 6.5 * COIN;} 
			else if (nHeight <= 460000) {nSubsidy = 6.25 * COIN;} 
			else if (nHeight <= 470000) {nSubsidy = 6 * COIN;} 
			else if (nHeight <= 480000) {nSubsidy = 5.75 * COIN;} 
			else if (nHeight <= 490000) {nSubsidy = 5.5 * COIN;} 
			else if (nHeight <= 500000) {nSubsidy = 5.25 * COIN;} 
			else if (nHeight <= 510000) {nSubsidy = 5 * COIN;} 
			else if (nHeight <= 520000) {nSubsidy = 4.75 * COIN;} 
			else if (nHeight <= 530000) {nSubsidy = 4.5 * COIN;} 
			else if (nHeight <= 540000) {nSubsidy = 4.25 * COIN;} 
			else if (nHeight <= 21000000) {nSubsidy = 4 * COIN;}
        ...
        int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
        {
			...
			else if (nHeight <= 40000) {ret = blockValue * 0.9;} 
			else if (nHeight <= 50000) {ret = blockValue * 0.9025;} 
			else if (nHeight <= 60000) {ret = blockValue * 0.905;} 
			else if (nHeight <= 70000) {ret = blockValue * 0.9075;} 
			else if (nHeight <= 80000) {ret = blockValue * 0.91;} 
			else if (nHeight <= 90000) {ret = blockValue * 0.9125;} 
			else if (nHeight <= 100000) {ret = blockValue * 0.915;} 
			else if (nHeight <= 110000) {ret = blockValue * 0.9175;} 
			else if (nHeight <= 120000) {ret = blockValue * 0.92;} 
			else if (nHeight <= 130000) {ret = blockValue * 0.9225;} 
			else if (nHeight <= 140000) {ret = blockValue * 0.925;} 
			else if (nHeight <= 150000) {ret = blockValue * 0.9275;} 
			else if (nHeight <= 160000) {ret = blockValue * 0.93;} 
			else if (nHeight <= 170000) {ret = blockValue * 0.9325;} 
			else if (nHeight <= 180000) {ret = blockValue * 0.935;} 
			else if (nHeight <= 190000) {ret = blockValue * 0.9375;} 
			else if (nHeight <= 200000) {ret = blockValue * 0.94;} 
			else if (nHeight <= 210000) {ret = blockValue * 0.9425;} 
			else if (nHeight <= 220000) {ret = blockValue * 0.945;} 
			else if (nHeight <= 230000) {ret = blockValue * 0.9475;} 
			else {ret = blockValue * 0.95;}
		...
		*/
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Beetok
        ...
        void RecalculateZABETSpent()
        {
            ...
            while (true) {
                ...
                //Remove spends from zBTOK supply
        ...
        {
            // Beetok: recalculate Accumulator Checkpoints that failed to database properly
 ...
 */
        ...
        void static UpdateTip(CBlockIndex * pindexNew)
        {
            ...
            // If turned on AutoZeromint will automatically convert BTOK to zBTOK
 ...
 */
        ...
        bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
        {
            ...
            if (nHeight > Params().Zerocoin_StartHeight()) {
                ...
                for (const CTransaction& tx : block.vtx) {
                    ...
                    // double check that there are no double spent zBTOK spends in this block
                    if (tx.IsZerocoinSpend()) {
                        for (const CTxIn txIn : tx.vin) {
                            if (txIn.scriptSig.IsZerocoinSpend()) {
                                libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                                if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                                    return state.DoS(100, error("%s : Double spending of zBTOK serial %s in block\n Block: %s",
        ...
        bool ProcessNewBlock(CValidationState & state, CNode * pfrom, CBlock * pblock, CDiskBlockPos * dbp)
        {
            ...
            if (nMints || nSpends)
                LogPrintf("%s : block contains %d zBTOK mints and %d zBTOK spends\n", __func__, nMints, nSpends);
        ...
        bool static ProcessMessage(CNode * pfrom, string strCommand, CDataStream & vRecv, int64_t nTimeReceived)
        {
            ...
            if (strCommand == NetMsgType::VERSION) {
                ...
                if (pfrom->cleanSubVer == "/Beetok Core:0.0.0.0/" ||
                    pfrom->cleanSubVer == "/Beetok Core:0.0.0.1/") {
                    LOCK(cs_main);
                ...
                // Beetok: We use certain sporks during IBD, so check to see if they are
