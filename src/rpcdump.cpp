...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Beetok address or script");
    }
...
UniValue dumpprivkey_abet(const UniValue& params, bool fHelp)
{
	...
	if (params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
	{
		std::string warning =
			...
			"Beetok team members will never ask for this command's output and it is not needed for masternode setup or diagnosis!\n";
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "they will have FULL ACCESS to STEAL your Beetok. Giving this file to someone\n"
            "is the same thing as giving them all of the Beetok in your wallet! Never send\n"
            "this file to ANYONE that you do not trust with all of your Beetok!!!\n"
            "\nArguments:\n"
...
/*    LOCK2(cs_main, pwalletMain->cs_wallet);
    ...
    // produce output
    file << strprintf("# Wallet dump created by Beetok %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue dumpallprivatekeys(const UniValue& params, bool fHelp)
{
	...
	// produce output
	file << strprintf("# Wallet private key dump file created by Beetok %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
	file << strprintf("# * Created on %s\n", EncodeDumpTime(GetTime()));
	file << strprintf("# * Best block at time of backup was %i (%s),\n", chainActive.Height(), chainActive.Tip()->GetBlockHash().ToString());
	file << strprintf("#   mined on %s\n", EncodeDumpTime(chainActive.Tip()->GetBlockTime()));
	file << "\n";
